...
## Important Notes
- **Hugo Version**: Must match in `mise.toml` and `vercel.json`. mise.toml takes precedence.
- **Goldmark**: Unsafe mode is enabled for raw HTML.
- **CI/CD**: GitHub Actions (`.github/workflows/gh-pages.yaml`) runs `./update.sh` weekly and on push.
